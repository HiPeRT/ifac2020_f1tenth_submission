...
void MurettoNode::scanCallback(const sensor_msgs::LaserScan::ConstPtr scan)
{
    try
    {
        tf_laser_to_map_ = tf_buffer_.lookupTransform("map2", "ego_racecar/prova", ros::Time(0));
    }
    ...
    geometry_msgs::Quaternion q = tf_laser_to_map_.transform.rotation;
    tf2::Quaternion tfq;
    tf2::fromMsg(q, tfq);
    tf2::Matrix3x3(tfq).getRPY(roll,pitch,yaw);

    //const double yaw = tf2::getYaw(tf_laser_to_map_.transform.rotation);
    ...
    double theta = scan->angle_min + angle_increment*(-1);
    ...
    for (int i = 0; i < scan->ranges.size(); ++i, angle += scan->angle_increment)
    {
        ...
        if(std::isinf(actual_range) || std::isnan(actual_range)) continue;

        // laser hit x, y in base_link frame
        const double x_base_link = actual_range*cos(theta);
        const double y_base_link = actual_range*sin(theta);

        if(x_base_link > 2 || y_base_link > 2) continue;
...
void MurettoNode::start()
{
    ...
    path = load_flag_path((trj_path + std::string("fattibile.txt"))); 
